...
import com.ekeyfund.oss.core.base.tips.ErrorTip;
...
public class GlobalExceptionHandler {
    ...
    public String credentials(InvalidKaptchaException e, Model model) {
        String username = HttpKit.getRequest().getParameter("username");
        switch(e.getReason()){
            case InvalidKaptchaException.EXPIRED:
                LogManager.me().executeLog(LogTaskFactory.loginLog(username, "验证码超时", HttpKit.getIp()));
                model.addAttribute("tips", "验证码超时");
                break;
            default:
                LogManager.me().executeLog(LogTaskFactory.loginLog(username, "验证码错误", HttpKit.getIp()));
                model.addAttribute("tips", "验证码错误");
        }
        model.addAttribute("tipstype", "kaptcha");
        return "/login.html";
    }

    @ExceptionHandler(AccountNotFoundException.class)
    public String handleAccountDeleteException(AccountNotFoundException accountDeleteException, Model model){
        String username = HttpKit.getRequest().getParameter("username");
        LogManager.me().executeLog(LogTaskFactory.loginLog(username, "账户不存在", HttpKit.getIp()));
