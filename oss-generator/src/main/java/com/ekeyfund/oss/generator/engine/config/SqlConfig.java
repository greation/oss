...
import com.ekeyfund.oss.core.constant.IsMenu;
...
public class SqlConfig {
    ...
    public void init() {
        ...
        //操作日志
        Menu oplog = createSubMenu(menu);
        oplog.setCode(contextConfig.getBizEnName() + "_oplog");
        oplog.setName(contextConfig.getBizChName() + "操作日志");
        oplog.setUrl("/" + contextConfig.getBizEnName() + "/oplog");
        menus.add(oplog);
    ...
    public String[] getPcodeAndPcodes() {
        ...
        Connection ossCon = null;
        try {
            //此处要用oss的数据源
            DynamicDataSource dds = SpringContextHolder.getBean("mutiDataSource");
            ossCon = dds.getDataSource(DatasourceEnum.DATA_SOURCE_YQ_OSS).getConnection();
            preparedStatement = ossCon.prepareStatement("select * from sys_menu where name like ?");
